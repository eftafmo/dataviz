    ...
    <div  v-if="hasData" class="sidebar-header">
      <transition name="fade">
        <div class="allocation" :key="transitioned">
          ...
          <small>
            {{ number(data.project_count_ended) }} projects completed<template v-if="data.project_count_positive">, of which {{ number(data.project_count_positive) }} ({{ data.project_percent_positive }}%) have had positive effects that are likely to continue beyond the funding period</template>
          </small>
        </div>
      </transition>
    </div>
