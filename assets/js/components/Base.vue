    ...
    aggregate (data, by, on, flatten = false) {
      ...
      for (const item of data) {
        ...
        for (let i = 0, j = _bycols.length; i < j; i++) {
          ...
          const dstcol = bycols[srccol]
          const value = item[srccol]
        ...
        for (const srccol in oncols) {
          ...
          const dstcol = _col.destination
          const type = _col.type
          const exclude = _col.exclude
          const exclude_empty = _col.exclude_empty
          const value = type === Number ? Number(item[srccol]) : item[srccol]
          ...
          if (type === Number) {
            ...
            if (current === undefined) { current = row[dstcol] = d3.set() }
