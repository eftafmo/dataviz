    ...
    <chart-container :width="width" :height="height">
      ...
      <div v-if="hasData && aggregated.allocation" class="info">
        <transition name="fade">
          ...
          >
            <p>
              <span class="amount">{{ currency(aggregated.allocation) }}</span>
              spent on
            </p>
          </div>
        </transition>
        <div class="data-wrapper">
          ...
          >
            <li class="programmes">
              <span class="amount">{{
                number(aggregated.programmes.size())
              }}</span>
              Programmes
            </li>
            <li class="projects">
              <span class="amount">{{ number(aggregated.project_count) }}</span>
              Projects
            </li>
          </ul>
        </div>
        <div :style="{ fontSize: fonts.bottom_text + 'px' }" class="ending">
          <p>
            ...
            strengthen bilateral relations in
            <span style="white-space: nowrap">{{ period }}</span> funding period
          </p>
        </div>
      </div>
...
export default Chart.extend({
  ...
  methods: {
    ...
    _highlight(index, yes) {
      ...
      links.filter((d, i) => i == index).classed("highlighted", yes);
      links.filter((d, i) => i != index).classed("non-highlighted", yes);
    },
