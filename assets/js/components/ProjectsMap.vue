...
const RegionDetails = {
  ...
  watch: {
    ...
    region (a, b) {
      ...
      if (a.project_count !== b.project_count) { this.changed_count = Number(!this.changed_count) }
    }
  }
}
...
export default AllocationMap.extend({
  ...
  methods: {
    ...
    tooltipTemplate (d) {
      ...
      const country_is_donor = d.id.length === 2 && this.COUNTRIES[d.id].type === 'donor'
      const state_type = country_is_donor ? 'donor-tooltip' : ''
    ...
    _domouse (over, d, i, group) {
      ...
      const selector = `${_selector}.level${level} > g.region.${id}`
    ...
    getBubbles (parentid) {
      ...
      const selector = this._getRegionSelector(parentid)

      if (this._rendered_bubbles[parentid]) { return this.projects.select(selector) }
      ...
      const _aggregate = (id, row) => {
        ...
        if (item === undefined) {
          ...
          for (const k in row) {
            ...
            if (v instanceof d3.set) { item[k] = d3.set(v.values()) } else { item[k] = v }
          ...
          for (const k in row) {
            ...
            if (typeof v === 'string') { continue } else if (typeof v === 'number') { item[k] += v } else if (v instanceof d3.set) { v.each(x => item[k].add(x)) } else { console.error('Unhandled key / value', k, v) }
          }
        }
      }
    ...
    handleFilterRegion (v, old) {
      ...
      this.current_region = v || beneficiary
      this.render()
    }
  }
})
