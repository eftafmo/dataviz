...
export default Chart.extend({
  ...
  methods: {
    ...
    renderDonorColours (t) {
      ...
      const no = this.FMS['norway-grants'].name

      const fmfilter = this.filters.fm

      if (fmfilter === eea) { with_eea = true } else if (fmfilter === no) { with_no = true } else {
    ...
    fillfunc (d, i, group) {
      ...
      const type = this.COUNTRIES[country].type

      if (type === 'donor') {
        if (country === 'NO') { return 'url(#multi-fm)' }

        if (this.filters.fm === 'Norway Grants') { return this.donor_colour_inactive }
      ...
      if (this.current_region) {
        ...
        if (this.isAncestorRegion(id, this.current_region)) { return this.ancestor_region_colour }
      }
