...
export default Chart.extend({
    ...
    filtered_sectors () {
      ...
      for (const sname in aggregated) {
        ...
        for (const aname in areas) {
          ...
          const area = sector.children[aid] = areas[aname]
    ...
    data () {
      ...
      if (sectortree === null) {
        ...
        for (const sid in sectortree) {
          ...
          const areas = d3.values(sector.children)
  ...
  methods: {
    ...
    areasBeforeEnter (el) {
      ...
      if (_idx === -1) { el.style.height = '0px' } else { this._areasCancelled.splice(_idx, 1) }
    ...
    areasBeforeLeave (el) {
      ...
      const _idx = this._areasCancelled.indexOf(el)
      if (_idx === -1) { el.style.height = el.clientHeight + 'px' } else { this._areasCancelled.splice(_idx, 1) }
    ...
    _mkcolourscale: (colour, length) => {
      ...
      const start = d3.hsl(_c.h, _c.s, _c.l, _c.opacity)
      const end = d3.hsl(_c.h, _c.s, _c.l, _c.opacity)
    ...
    tooltipTemplate (d) {
      ...
      let bss = d.data.beneficiaries
      let prgs = d.data.programmes
    ...
    renderChart () {
      ...
      // flatten data
      const data = this._partition(this.data)
        .descendants()
        .slice(1)
    ...
    handleFilterArea (val) {
      if (val !== null && this.filters.sector === null) { console.error('Filtered by area without a sector. Impossible 2.') }
    ...
    calcHeight () {
      if (window.matchMedia('(min-width: 768px)').matches) {
        ...
        if (!legend) return
        const prevHeight = legend.clientHeight
