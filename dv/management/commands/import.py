...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not funding_period or funding_period == "2009-2014":
            if directory:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        "A directory containing xlsx files must "
    ...
    def clean_for_period(self, funding_period, noinput):
        self.stdout.write(
            f"Removing data for period {funding_period} from {settings.DB_PATH}"
        )
    ...
    def _import_2004_2009(self, json_path):
        ...
        sector, created = PrioritySector.objects.get_or_create(
            {"code": "XX0", "name": "Fake sector 2004-2009"}, code="XX0"
        )
        self.stdout.write(
            self.style.SUCCESS(f"Imported {int(created)} PrioritySector objects.")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Imported {int(created)} ProgrammeArea objects.")
        )
    ...
    def _import_2009_2014(self, directory_path):
        ...
        files = [
            ...
            if (
                ...
                and f.endswith(".xlsx")
                and f[:-5].lower() in (name.lower() for name in EXCEL_FILES)
            )
        ]
        if not files:
        ...
        for record in sheet.records:
            achievement_eea = (
                record["Achievement"] if record["FMCode"] == "EEA FM" else 0
            )
            achievement_norway = (
    ...
    def _import_2014_2021(self):
        ...
        with db_cursor() as cursor:
            ...
            for row in cursor.fetchall():
                ...
                self._add_m2m_entries(
                    programme, row, "Country", "states", "State", states
                )
        ...
        with db_cursor() as cursor:
            ...
            for row in cursor.fetchall():
                project = Project.objects.create(
                    ...
                    initial_description=sanitize_html(
                        row["ProjectInitialDescriptionHtml"]
                    ),
                    results_description=sanitize_html(
                        row["ProjectResultsDescriptionHtml"]
                    ),
                    thematic=row["Thematic"] or "",
                )
        ...
        with db_cursor() as cursor:
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Error importing organisation {row['Organisation']}."
                        )
                    )
        ...
        with db_cursor() as cursor:
            ...
            for row in cursor.fetchall():
                ...
                self._add_m2m_entries(
                    bilateral_initiative,
                    row,
                    "ProgrammeAreaCodesList",
                    "programme_areas",
                    "ProgrammeArea",
                    programme_areas,
                )
    ...
    def _add_m2m_entries(self, obj, row, key, m2m_attr, m2m_attr_type, m2m_list):
        for code in (row[key] or "").split(","):
            ...
            if code and not m2m_obj:
                self.stdout.write(
                    self.style.WARNING(f"{m2m_attr_type} {code} not found.")
                )
                continue
