...
def overview(request):
    ...
    if period_id == 2:
        ...
        bilateral_fund = {
            (bf["financial_mechanism"], bf["state"]): bf["allocation"]
            for bf in allocations.filter(
                programme_area__code__in=("TA02", "TA04", "OTBF")
            )
        }
...
def grants(request):
    ...
    if period_id == 2:
        ...
        bilateral_fund = {
            (bf["financial_mechanism"], bf["state"], bf["programme_area"]): bf[
                "allocation"
            ]
            for bf in programme_allocations
        }
        ...
        bilateral_fund = {
            (
                bf.financial_mechanism,
                bf.state_id,
                bf.programme_area_id,
            ): bf.gross_allocation
            for bf in allocations.filter(
                programme_area__code__in=("TA02", "TA04", "OTBF")
            )
        ...
        out.append(
            {
                ...
                "bilateral_allocation": bilateral_fund.get(
                    (financial_mechanism, state, programme_area), 0
                ),
...
def sdg(request):
    ...
    allocation_query = (
        ProgrammeAllocation.objects.filter(
            funding_period=period_id, sdg_no__isnull=False
        )
        .exclude(
            allocation=0,
        )
...
def projects(request):
    ...
    news_query = (
        News.objects.filter(
            Q(project__funding_period=period_id)
            | Q(programmes__funding_period=period_id),
        )
        .prefetch_related(
            "project",
...
def partners(request):
    ...
    programme_donors_query = (
        OrganisationRole.objects.filter(
            funding_period=period_id,
                        ...
                        for src in project_nuts[prj_code]["src"]:
                            ...
                            for dst in project_nuts[prj_code]["dst"]:
                                ...
                                if src not in projects[key_donor][prj_code]["src_nuts"]:
                                    projects[key_donor][prj_code]["src_nuts"].append(
                                        src
                                    )
                                if dst not in projects[key_donor][prj_code]["dst_nuts"]:
                                    projects[key_donor][prj_code]["dst_nuts"].append(
                                        dst
                                    )
                    row = {
                        "fm": pa_data["fm"],
                    ...
                    if donor_programme_partners[(prg, donor)]:
                        ...
                        if row["PJDPP"]:
                            if "PO" in row:
                                del row[
                                    "PO"
                                ]  # Apparently we only need PO's for DPP programmes
                            out.append(row)
    return JsonResponse(out)
...
def sdg_beneficiary_detail(request, beneficiary):
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Beneficiary state '{state_id}' does not exist."}, status=404
        )
    ...
    nuts3s = tuple(
        NUTS.objects.filter(nuts_versions__year=NUTS_VERSION_BY_PERIOD[period])
    ...
    for pa in project_allocation_query:
        ...
        children = (
            nuts3s if len(code) == 2 else [n for n in nuts3s if n.startswith(code)]
        )
    ...
    def get_queryset(self):
        ...
        if is_dpp and donor:
            ...
            if donor_name:
                ...
                q &= ~Q(
                    organisation_roles__organisation__country__in=EEA_DONOR_STATES.keys()
                )
